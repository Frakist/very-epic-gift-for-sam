...
class PlayState extends MusicBeatState
{
	...
	var sadSam:FlxSprite;

	var sinMod:Float = 0;
	var decoAnim:FlxSprite;
	var backgroundweird:FlxSprite;
	...
	override public function create()
	{
			...
			switch (SONG.song.toLowerCase())
			{
				...
				case 'interrupted':
					{
						...
						var background:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('background', 'week7'));
						background.setGraphicSize(Std.int(background.width * background.height));
						background.antialiasing = true;

						decoAnim = new FlxSprite(-50, -25).loadGraphic(Paths.image('Shape 3', 'week7'));
						decoAnim.antialiasing = true;
						decoAnim.updateHitbox();
						decoAnim.antialiasing = true;
						decoAnim.scale.set(2.5, 1.9);


						var floor:FlxSprite = new FlxSprite(-650, -350).loadGraphic(Paths.image('floor', 'week7'));
				...
				case 'insanity':
					{						
					    ...
					    var background:FlxSprite = new FlxSprite().makeGraphic(1920, 1080, 0xFF818181);
						background.updateHitbox();
						background.setGraphicSize(Std.int(background.width * background.height));

						decoAnim = new FlxSprite(-50, -75).loadGraphic(Paths.image('Shape 3', 'week7'));
						decoAnim.antialiasing = true;
						decoAnim.updateHitbox();
						decoAnim.antialiasing = true;
						decoAnim.scale.set(2.5, 1.9);
						decoAnim.color = 0xFF919191;
						...
						sadSam = new FlxSprite(-150, 450);
						sadSam.frames = Paths.getSparrowAtlas('SamBackgroundSprites', 'week7');
						sadSam.animation.addByPrefix('samBob', 'SamIdle', 24, false);
						sadSam.antialiasing = true;
						sadSam.updateHitbox();
						add(sadSam);
				...
				case 'onslaught':
					{
					    ...
					    var background:FlxSprite = new FlxSprite().makeGraphic(1920, 1080, 0xFF341C1C);
						background.updateHitbox();
						background.setGraphicSize(Std.int(background.width * background.height));
						...
						decoAnim.antialiasing = true;
						decoAnim.scale.set(2.5, 1.9);
						decoAnim.color = 0xFF181818;

						var floor:FlxSprite = new FlxSprite(-650, -350).loadGraphic(Paths.image('floor', 'week7'));
						floor.setGraphicSize(Std.int(floor.width * 2));
						floor.antialiasing = true;
						floor.updateHitbox();
						floor.color = 0xFF020202;
						add(background);
						add(decoAnim);
						add(floor);
					}
				case 'conquered':
					{
						...
						backgroundweird = new FlxSprite();
						backgroundweird.frames = Paths.getSparrowAtlas('cockuered', 'week7');
						backgroundweird.animation.addByPrefix('beat', 'beat', 24, false);
						backgroundweird.antialiasing = true;
						backgroundweird.updateHitbox();

						var ground:FlxSprite = new FlxSprite(0, -50).loadGraphic(Paths.image('ground bruh', 'week7'));
						ground.antialiasing = true;
						ground.updateHitbox();

						backgroundweird.scale.set(3, 3);
		...
		switch (curStage)
		{
			...
			case 'mallEvil':
				boyfriend.x += 320;
				dad.y -= 80;
			case 'cum' | 'void' | 'angryvoid' | 'onslaught':
				gf.y += 325;
				boyfriend.y+= 100;
			case 'schoolEvil':
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				...
				case 'cum' | 'onslaught':
					babyArrow.frames = Paths.getSparrowAtlas('cum', 'shared');
	...
	override public function update(elapsed:Float)
	{
		...
		**/
		...
		switch (curStage)
		{
			...
			case 'void' | 'angryvoid':
				sinMod += 0.007;
				decoAnim.y = Math.sin(sinMod) * 40 + 200;
				decoAnim.alpha = Math.sin(sinMod) * 1 + 0;
			case 'onslaught':
				sinMod += 0.025;
				decoAnim.y = Math.sin(sinMod) * 40 + 200;
				decoAnim.alpha = Math.sin(sinMod) * 1 + 0;

				var thisX:Float = Math.sin(sinMod / 2) * 100;
				var thisY:Float = Math.sin(sinMod) * 50;
		...
		if (curSong == 'insanity')
			{
				if(curStep == 199)
					{
						remove(dad);
						dad = new Character(200, 200, 'madglitch');
						add(dad);
					}
				if(curStep == 203)
					{
						dad.animation.play('angry', true);
					}
			}
	...
	override function beatHit()
	{
		...
		if (curSong == 'onslaught' && curBeat >= 0 && curBeat <= 64)
			{
				FlxG.camera.zoom += 0.050;
			}
		if (curSong == 'onslaught' && curBeat >= 127 && curBeat <= 208)
			{
				FlxG.camera.zoom += 0.050;
			}
		if (curSong == 'onslaught' && curBeat >= 255)
			{
				FlxG.camera.zoom += 0.050;
			}
		...
		if (!_variables.chromakey)
		{
			switch (curStage)
			{
				...
				case 'cum':
					backgroundweird.animation.play('beat');
					
				case 'angryvoid':
					sadSam.animation.play('samBob');
